...
def test_find_orbit4(engine, ml_lattice, py_lattice, dp, refpts):
    ...
    ml_orbit4, _ = engine.findorbit4(ml_lattice, dp, ml_refpts, nargout=2)
    # Select the first refpt.
    py_ml_orbit4 = numpy.asarray(ml_orbit4)[:, 0]

    # Python call
    py_orbit4, _ = physics.find_orbit4(py_lattice, dp, refpts)

    numpy.testing.assert_almost_equal(py_ml_orbit4, py_orbit4)
